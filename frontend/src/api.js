...
function formatApiError(detail, status) {
  ...
  if (detail?.message) return typeof detail.message === "string" ? detail.message : JSON.stringify(detail.message);
...
async function blobRequest(path, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401 && token) window.dispatchEvent(new Event("kairo:unauthorized"));
      const detail = data?.detail;
      const message = typeof detail === "string" ? detail : (typeof detail?.message === "string" ? detail.message : Array.isArray(detail) ? formatApiError(detail, res.status) : `Request failed (${res.status})`);
      const error = new Error(message);
      error.status = res.status; error.code = detail?.code; error.detail = detail;
      throw error;
    }
...
export const api = {
  ...
  dashboard: async ()=>{const h=await request("/api/health",{timeout:10000});return {cases:h.counts?.cases??0,documents:h.counts?.evidence??0,versions:"—",audit_events:h.counts?.events??0};},
  ...
  caseMembers: id=>request(`/api/cases/${id}/members`),
  ...
  documents: async caseId=>(await request(`/api/cases/${caseId}`)).evidence||[],
  search: async (q="",filters={})=>{const p=new URLSearchParams();if(q.trim())p.set("q",q.trim());p.set("limit",filters.limit||50);const rows=await request(`/api/search?${p.toString()}`);return rows.map(x=>({...x,document_id:x.id,document_number:x.evidence_no,document_type:x.kind,current_version:x.current_version,case_number:x.case_no,case_id:x.case_id,case_title:x.case_title||"",filename:x.filename||"",sha256:x.sha256||""}));},
  versions: async docId=>(await request(`/api/evidence/${docId}`)).versions||[],
